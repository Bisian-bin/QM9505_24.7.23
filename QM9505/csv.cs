...
namespace QM9505
{
    public class csv
    {
        ...
        public static DataTable GetDgvToTable(DataGridView dgv)//绑定DataGridView数据到DataTable
        {
        ...
        public static void SaveCSV(DataTable dt, string fullPath)//table数据写入csv
        {
            try
            {
                ...
                {
                    data += dt.Columns[i].ColumnName.ToString();//datagrid1.Columns[i].HeaderText
        ...
        public static DataTable OpenCSV(string filePath)//从csv读取数据返回table
        {
            ...
            try
            {
                if (File.Exists(filePath))   //判断数据库是否存在
                {
                    //存在 
