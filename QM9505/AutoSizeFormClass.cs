...
namespace QM9505
{
    class AutoSizeFormClass
    {
        ...
        public List<controlRect> oldCtrl = new List<controlRect>();//列表
        ...
        public void controlAutoSize(Control mForm)
        {
            ...
            float new_len = (float)Math.Sqrt(mForm.Width * mForm.Width + mForm.Height * mForm.Height);
            float old_len = (float)Math.Sqrt(oldCtrl[0].Width * oldCtrl[0].Width + oldCtrl[0].Height * oldCtrl[0].Height);

            if (mForm.Width != oldCtrl[0].Width && mForm.Height != oldCtrl[0].Height)
                FontMagnification = new_len / old_len;
            else
                FontMagnification = 1;
        ...
        private void AutoScaleControl(Control ctl, float wScale, float hScale)
        {
            ...
            foreach (Control c in ctl.Controls)
            {
                float size_chg = oldCtrl[ctrlNo].size * FontMagnification;
                FontFamily ff = new FontFamily(c.Font.Name);
                c.Font = new Font(ff, size_chg, oldCtrl[ctrlNo].fontstyle);
