...
namespace QM9505
{
    class PhotoTcpServer
    {
        ...
        public static bool StartListening()
        {
            ...
            try
            {
                IPAddress ip = IPAddress.Parse(Variable.serverIP1);     //服务器IP地址
                tcpListener = new TcpListener(ip, Convert.ToInt32(Variable.serverport1));     //服务器端口号
        ...
        public static void StartListen()
        {
            try
            {
                while (true)
                {
                    ...
                    Variable.Server1Connect = true;
                    Variable.clientIP1 = array[0];       //显示客户端IP
                    Variable.clientport1 = array[1];   //显示客户端端口号
        ...
        public static void MessageSend(string str)
        {
            try
            {
                ...
                {
                    //MessageBox.Show("相机客户端Unconnected");
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                while (true)
                {
                    ...
                    else
                    {
                        ...
                        //显示信息
                        Variable.PhotoRecMessage = RecMessage;
                    }
                }
            }
            catch (Exception ex)
